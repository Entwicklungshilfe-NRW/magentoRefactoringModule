 ...
 * @package    Testing_Carrier
 */
class Testing_Carrier_Model_Pdf_Label extends Testing_Carrier_Model_Pdf_Abstract{
	...
	public function createLabel($tagdata){
		...
		foreach($tagdata as $item) {
			if ($item->getValue() != null) {
				if(get_class($item->getItem()) === 'Testing_Carrier_Model_Label_Item_Font'  ){
					$this->drawFont($item);
				}
				if(get_class($item->getItem()) === 'Testing_Carrier_Model_Label_Item_Barcode'  ){
					$this->drawBarcode($item);
				}
				if(get_class($item->getItem()) === 'Testing_Carrier_Model_Label_Item_Datamatrix'  ){
